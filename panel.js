...
const DEFAULT_COACHES = [
  ...
  { id: "marti", name: "Mart\xED", role: "Nutricionista", color: "#65A30D", email: "" },
  { id: "marina", name: "Marina", role: "Psic\xF3loga", color: "#9333EA", email: "" }
];
const DEFAULT_ROOMS = [
...
function AxisPanel() {
    ...
    try {
      ...
      setSessions((prev) => {
        ...
        for (const s of allRecords) {
          ...
          map.set(s.id, old ? { ...s, sent: old.sent } : s);
        }
        return [...map.values()];
      });
      setAutoSyncState({
        status: "done",
        msg: `\xDAltima sincronizaci\xF3n: ${(/* @__PURE__ */ new Date()).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })} \xB7 ${allRecords.length} sesi\xF3n(es) cargadas.`
      });
    } catch (e) {
  ...
  function handleSend(toSend) {
    ...
    const sinEmail = toSend.filter((s) => {
      var _a;
      return !((_a = coachOf(s)) == null ? void 0 : _a.email);
    });
    try {
      ...
      setSendState({
        status: "done",
        msg: `Descargado axis-${date}.ics con ${toSend.length} evento(s). \xC1brelo (doble clic) y tu calendario (Google/Outlook/Apple) lo importar\xE1 solo.${sinEmail.length ? ` Ojo: ${sinEmail.length} sesi\xF3n(es) sin email de coach \u2014 config\xFAralo en la pesta\xF1a Equipo para que se le invite autom\xE1ticamente al abrir el archivo.` : ""}`
      });
    } catch (e) {
  ...
  const updateSession = (id, patch) => setSessions((p) => p.map((s) => s.id === id ? { ...s, ...patch } : s));
  ...
  }), importPreview.length > 8 && /* @__PURE__ */ React.createElement("div", { style: { color: "#5A6B63", paddingTop: 4 } }, "\u2026y ", importPreview.length - 8, " m\xE1s"))), /* @__PURE__ */ React.createElement("section", { style: { background: "#fff", border: "1px solid #DDE4E0", borderRadius: 12, padding: 16 } }, /* @__PURE__ */ React.createElement("h2", { className: "axis-display", style: { margin: "0 0 6px", fontSize: 20, textTransform: "uppercase" } }, "Importar pagos (CSV)"), /* @__PURE__ */ React.createElement("p", { style: { margin: "0 0 10px", fontSize: 13, color: "#5A6B63" } }, 'Pega el contenido del export de AimHarder ("Pagos finalizados" o "Pagos pendientes", separado por punto y coma). Detecta duplicados por ID; si un pago pendiente aparece luego como finalizado, se marca como cobrado autom\xE1ticamente. Ahora mismo hay ', /* @__PURE__ */ React.createElement("b", null, payments.length), " pagos cargados."), /* @__PURE__ */ React.createElement(
    "textarea",
    {
      className: "axis-input",
      rows: 5,
