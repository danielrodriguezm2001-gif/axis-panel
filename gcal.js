...
(function () {
  ...
  var dayEventsCache = {};
  async function listAxisEvents(dateISO) {
    if (dayEventsCache[dateISO]) return dayEventsCache[dateISO];
    try {
      ...
      if (res.ok) {
        ...
        dayEventsCache[dateISO] = (data.items || []).filter(function (ev) {
          return (ev.summary || "").indexOf("Axis ·") === 0 && ev.start && ev.start.dateTime;
        });
        return dayEventsCache[dateISO];
      }
    } catch (e) {}
    dayEventsCache[dateISO] = [];
    return dayEventsCache[dateISO];
  }
  ...
  async function sendPending() {
    ...
    if (!toSend.length) return { ok: 0, linked: 0, fail: 0, noEmail: 0 };

    dayEventsCache = {};
    var coaches = readJSON("axis-coaches", []);
    var rooms = readJSON("axis-rooms", []);
    var ok = 0, linked = 0, fail = 0, noEmail = 0;
    ...
    for (var i = 0; i < toSend.length; i++) {
      ...
      /* Antiduplicados: si ya hay un evento idéntico en el Calendar
         (mismo título y misma hora de inicio), se reutiliza y NO se crea otro. */
      var existing = null;
      var dayEvents = await listAxisEvents(s.date);
      for (var j = 0; j < dayEvents.length; j++) {
        if (dayEvents[j].summary === body.summary &&
            dayEvents[j].start.dateTime.indexOf(s.date + "T" + s.start) === 0) {
          existing = dayEvents[j];
          break;
        }
      }
      if (existing) {
        map[String(s.id)] = { eventId: existing.id, date: s.date };
        writeMap(map);
        s.gcalSent = true;
        linked++;
        continue;
      ...
      try {
        ...
        if (res.ok) {
          ...
          writeMap(map);
          s.gcalSent = true;
          /* añadirlo a la caché del día para no duplicar dentro del mismo ciclo */
          dayEventsCache[s.date] = (dayEventsCache[s.date] || []).concat([{ id: ev.id, summary: body.summary, start: { dateTime: s.date + "T" + s.start + ":00" } }]);
    ...
    return { ok: ok, linked: linked, fail: fail, noEmail: noEmail };
  }
  ...
  async function runCycle() {
    ...
    try {
      ...
      if (sent.linked) parts.push(sent.linked + " ya estaban en el Calendar (vinculadas, sin reenviar)");
